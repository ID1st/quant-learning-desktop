...
- Plugin package management and strategy-runtime isolation are complete: Electron main-process installation into a managed local directory, manifest and version validation, allow-listed permissions, enable/disable/uninstall controls, typed preload IPC, and an Electron Utility Process host. Installed strategy source never reaches the renderer; the host has a capability-only registration protocol, import rejection, size/time/output limits, process termination, and failure recording. Indicator/data-source/export execution remains deferred until dedicated isolated contracts exist.
- The production-built plugin runtime is covered by `npm run smoke:plugin-runtime`, which launches Electron and verifies actual Utility Process activation and strategy execution rather than only a Node mock.
