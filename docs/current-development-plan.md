...
- Minimal UTORB and Trend Targets strategy implementations.
  - `chengzuopeng/stock-sdk` is the primary source for CN/HK/US quote snapshots and CN/HK current-session timelines. Current-network testing confirms that its Eastmoney-backed daily, weekly, and US minute K-line routes can be unreachable, so US chart bars are routed to Yahoo Finance first and CN/HK daily/weekly bars require a verified credential-backed fallback when that upstream is unavailable.
