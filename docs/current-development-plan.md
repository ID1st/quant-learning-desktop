...
- Chart workspace exposes `realtime`, `1d`, and `1w`; realtime depends on AlphaFeed `1m` historical-bar permission and degrades with a clear no-permission state when unavailable.
...
5. Initial sync fetches AlphaFeed K-line bars for available default periods. The chart workspace renders `realtime`, `1d`, and `1w`, with realtime backfilled on demand from `1m` bars.
...
12. On the `realtime` chart, the active symbol first loads AlphaFeed 1m historical bars into the `realtime` cache. During market hours, quote snapshots can append new points; after close, polling stops and only the historical intraday line remains.
...
### 2. LongBridge Backup Expansion

Goal: make LongBridge an explicit fallback path for quotes and future broker/account workflows.

Acceptance:

- Retry quote snapshots through LongBridge when AlphaFeed is unavailable and backup credentials exist.
- Surface which provider served the current quote snapshot.
