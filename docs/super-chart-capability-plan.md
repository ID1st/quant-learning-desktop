...
- 基础绘图支持趋势线、水平线和文本标注，并按 `market + symbol + timeframe` 持久化。新增命令模型支持 add/update/delete/clear 以及撤销、重做；趋势线端点、水平线和文本标注均可在画布内选择和拖拽编辑。
- 图表增加回到最新与锁定价格比例。实时更新继续保留现有视图同步策略，不会重置手动平移、右侧留白或价格比例。
- 信号面板可选择单个策略信号，查看关联策略、参数、最新日志、时间和价格，并可跳转到策略配置。

本轮明确延后：矩形/斐波那契/测量工具、副图指标的独立面板、图层拖拽排序和跨设备绘图同步。它们将建立在已落地的命令与渲染协议上，而不改变策略或数据源架构。
