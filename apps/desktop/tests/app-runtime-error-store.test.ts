...
test("runtime error records dedupe immediate repeats and keep the newest entries", () => {
  const first = createAppRuntimeErrorRecord(
    "render",
    new Error("render failed"),
    "2026-07-13T08:00:00.000Z",
  );
  const duplicate = createAppRuntimeErrorRecord(
    "render",
    new Error("render failed"),
    "2026-07-13T08:00:03.000Z",
  );
  const next = createAppRuntimeErrorRecord(
    "unhandled-rejection",
    new Error("request failed"),
    "2026-07-13T08:00:10.000Z",
  );

  const deduped = appendAppRuntimeError([first], duplicate);
  assert.equal(deduped.length, 1);
  assert.deepEqual(
    appendAppRuntimeError(deduped, next).map((item) => item.scope),
    ["unhandled-rejection", "render"],
  );
});
