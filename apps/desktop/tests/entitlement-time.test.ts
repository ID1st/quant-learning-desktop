...
test("formats entitlement time with the two largest useful units", () => {
  ...
  assert.equal(formatEntitlementRemaining("2026-08-01T14:30:00.000Z", now), "2 天 2 小时");
  assert.equal(formatEntitlementRemaining("2026-07-30T12:45:00.000Z", now), "45 分钟");
});

test("handles near-expiry, expired, and invalid entitlement times", () => {
  const now = new Date("2026-07-30T12:00:00.000Z");

  assert.equal(formatEntitlementRemaining("2026-07-30T12:00:30.000Z", now), "不足 1 分钟");
