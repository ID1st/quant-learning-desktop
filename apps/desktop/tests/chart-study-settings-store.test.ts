...
import { builtInChartIndicatorDefinitions, getIndicatorInstance, setIndicatorEnabled, updateIndicatorInstance } from "../src/features/chartIndicators/chartIndicators.ts";
import {
...
test("chart study settings migrate legacy workspace strategy and indicator preferences", () => {
  ...
  const ma = getIndicatorInstance(store.getState().indicators, "ma", builtInChartIndicatorDefinitions[0]);
  assert.deepEqual(
    [ma.parametersByConvention["a-share"].period1, ma.parametersByConvention["a-share"].period2, ma.parametersByConvention["a-share"].period3, ma.parametersByConvention["a-share"].period4],
    [21, 10, 20, 60],
  );
  assert.equal(getIndicatorInstance(store.getState().indicators, "vol").enabled, true);
  assert.equal(getIndicatorInstance(store.getState().indicators, "boll", builtInChartIndicatorDefinitions[1]).enabled, true);
  assert.equal(getIndicatorInstance(store.getState().indicators, "boll", builtInChartIndicatorDefinitions[1]).visible, false);
  assert.deepEqual(
    getIndicatorInstance(store.getState().indicators, "boll", builtInChartIndicatorDefinitions[1]).parametersByConvention["a-share"],
    getIndicatorInstance(store.getState().indicators, "boll", builtInChartIndicatorDefinitions[1]).parametersByConvention["cross-market"],
  );
});
...
test("chart study settings persist strategy and indicator updates for both workspaces", () => {
  ...
  store.getState().updateIndicators((current) => updateIndicatorInstance(setIndicatorEnabled(current, "ma", true), "ma", (item) => ({
    ...item,
  ...
  assert.equal(getIndicatorInstance(reloaded.getState().indicators, "ma", builtInChartIndicatorDefinitions[0]).enabled, true);
  assert.equal(getIndicatorInstance(reloaded.getState().indicators, "ma", builtInChartIndicatorDefinitions[0]).parametersByConvention["a-share"].period1, 50);
});
...
test("secondary indicator exclusivity and convention-specific parameters survive reload", () => {
      ...
      instances: {
        ...
        rsi: {
          ...
          parametersByConvention: {
            ...
            "cross-market": { ...withRsi.instances.rsi!.parametersByConvention["cross-market"], period1: 16 },
          },
        },
      },
    };
  ...
  assert.equal(getIndicatorInstance(reloaded.getState().indicators, "rsi").parametersByConvention["cross-market"].period1, 16);
  assert.equal(getIndicatorInstance(reloaded.getState().indicators, "rsi").parametersByConvention["a-share"].period1, 6);
});
