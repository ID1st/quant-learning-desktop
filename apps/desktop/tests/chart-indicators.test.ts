...
test("new users start with every indicator disabled", () => {
  assert.equal(
    builtInChartIndicatorDefinitions.every(
      (definition) =>
        getIndicatorInstance(defaultChartIndicatorSettings, definition.id, definition).enabled ===
        false,
    ),
    true,
  );
...
test("indicator evaluations honor enabled and visible lifecycle states", () => {
  ...
  const settings = updateIndicatorInstance(
    setIndicatorEnabled(defaultChartIndicatorSettings, "boll", true),
    "boll",
    (current) => ({ ...current, visible: false }),
    builtInChartIndicatorDefinitions[1],
  );
...
test("indicator registry exposes a plugin-safe registration boundary", () => {
  ...
  registry.register({
    id: "plugin-test",
    name: "插件测试指标",
    parameters: [{ key: "window", label: "周期", type: "number", defaultValue: 20 }],
    evaluate: () => null,
  });
  assert.equal(registry.get("plugin-test")?.parameters[0]?.key, "window");
  assert.throws(
    () =>
      registry.register({ id: "plugin-test", name: "重复", parameters: [], evaluate: () => null }),
    /重复指标注册/,
  );
});
...
test("legacy plugins without placement remain compatible as overlays and failures stay isolated", () => {
  const candles = [
    { time: "1", timestamp: 1, open: 100, high: 101, low: 99, close: 100, volume: 1 },
  ];
  const definitions = [
    {
      ...
      evaluate: (_items, parameters) => ({
        id: "plugin-line",
        name: String(parameters.window),
        source: "indicator",
        enabled: true,
        visible: true,
        zIndex: 30,
    ...
    {
      id: "com.quant.indicator.sample:broken",
      name: "Broken",
      parameters: [],
      evaluate: () => {
  ...
  const pluginSettings = updateIndicatorInstance(
    defaultChartIndicatorSettings,
    "com.quant.indicator.sample:line",
    (current) => ({
      ...current,
      enabled: true,
  ...
  const evaluations = createChartIndicatorEvaluations(
    candles,
    pluginSettings,
    "cross-market",
    definitions,
  ...
  assert.deepEqual(
    evaluations.map((evaluation) => evaluation.placement),
    ["overlay"],
  );
...
test("plugin panes participate in secondary exclusivity and keep persisted settings", () => {
  ...
  settings = updateIndicatorInstance(
    settings,
    pluginPane.id,
    (current) => ({
      ...current,
  ...
  assert.equal(
    getIndicatorInstance(restored, pluginPane.id, pluginPane).parametersByConvention["a-share"]
      .period,
    13,
  );
  assert.equal(
    getIndicatorInstance(restored, pluginPane.id, pluginPane).parametersByConvention["cross-market"]
      .period,
    21,
  );
});
