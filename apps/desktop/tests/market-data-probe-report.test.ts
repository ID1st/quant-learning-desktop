...
test("market data probe classifies actionable upstream failures without exposing raw errors", () => {
  assert.equal(
    classifyMarketDataProbeFailure("Tencent Finance request failed with HTTP 429."),
    "rate-limited",
  );
  assert.equal(classifyMarketDataProbeFailure("AlphaFeed permission denied."), "unauthorized");
  assert.equal(
    classifyMarketDataProbeFailure(
      "Tencent Finance returned no usable historical data for AAPL.US.",
    ),
    "no-data",
  );
  assert.equal(
    classifyMarketDataProbeFailure("Partial weekly historical series: 5 of 26 rows."),
    "insufficient-history",
  );
  assert.equal(
    classifyMarketDataProbeFailure("Partial 1w series: 5 of 26 rows."),
    "insufficient-history",
  );
  assert.equal(
    classifyMarketDataProbeFailure("Discontinuous daily historical series: gap is 300 days."),
    "discontinuous-history",
  );
  assert.equal(
    classifyMarketDataProbeFailure("Discontinuous 1d series: gap is 300 days."),
    "discontinuous-history",
  );
  assert.equal(classifyMarketDataProbeFailure("Expected at least 1 bars, received 0."), "no-data");
  assert.equal(
    classifyMarketDataProbeFailure("Stock SDK request failed: fetch failed."),
    "network",
  );
...
test("market data probe rejects sparse or discontinuous daily and weekly history", () => {
  const weekly = Array.from({ length: 5 }, (_, index) => ({
    timestamp: Date.UTC(2026, 0, 1 + index * 7),
  }));
