...
import {
  aggregateBarsToTimeframe,
  createStrategySeriesByTimeframe,
} from "../src/features/strategies/strategySeries.ts";

function createMinuteBars(startTimestamp: number, count: number) {
...
const minuteBars = createMinuteBars(Date.UTC(2026, 6, 2, 13, 30), 60);
...
describe("strategy timeframe series", () => {
  ...
  it("anchors US hourly bars at 09:30 and emits the closed 30-minute tail", () => {
    const bars = createMinuteBars(Date.UTC(2026, 6, 2, 13, 30), 390);
