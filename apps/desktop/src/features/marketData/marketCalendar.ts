...
export type TradingCalendarResolution =
  | { ok: true; session: TradingSession | null }
  | { ok: false; errorCode: TradingCalendarErrorCode };
...
export function resolveTradingDate(
  ...
  return {
    ...
    session: {
      ...
      segments: profile.halfDays.has(date)
        ? profile.halfDaySegments
        : profile.regularSegments,
...
export function resolveTradingSessionAt(
  ...
  return resolveTradingDate(
    market,
    getZonedDateParts(new Date(timestamp), profile.timeZone),
  );
}
...
export function getTradingSegmentAt(
  market: Market,
  timestamp: number,
) {
