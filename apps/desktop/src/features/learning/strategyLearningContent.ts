...
export const strategyLearningEntries: readonly StrategyLearningEntry[] = [
  {
    ...
    sections: [
      ...
      { title: "图表如何表达", content: "区间带、高低线及上下六段扩展区域会持续到所选绘制结束时间；最新目标标签直接显示历史命中率。突破标签同时标注相对 20 根均量的高量/低量，移动风险线随 ATR 单向收紧。" },
    ...
    chartOutputs: ["高量/低量突破标签", "逐日开盘区间", "六段扩展区域", "命中率目标标签", "成交量分布", "ATR 移动风险线"],
  ...
  },
  {
    ...
    placement: "主图叠加",
    sections: [
      { title: "计算含义", content: "MA 会将最近 N 个收盘价平均，每个价格的权重相同；系统默认同时显示 MA5/10/20/60。" },
      { title: "图表含义", content: "价格在均线上方或下方可用于观察趋势倾向；均线本身不直接构成买卖结论。" },
      { title: "常见局限", content: "横盘时价格会频繁穿越均线，容易出现噪声。" },
    ],
    parameters: [{ name: "均线周期", defaultValue: "5 / 10 / 20 / 60", description: "每条线完成完整窗口预热后才开始显示。周期越大，曲线越平滑、反应越慢。" }],
    chartOutputs: ["四条趋势均线"],
