...
export function resolveAuthBaseUrl(injectedBaseUrl: string, isPackaged: boolean): string {
  const configuredBaseUrl = injectedBaseUrl.trim();
  if (configuredBaseUrl) {
    return configuredBaseUrl;
  }
  return isPackaged ? PRODUCTION_AUTH_BASE_URL : DEVELOPMENT_AUTH_BASE_URL;
}
