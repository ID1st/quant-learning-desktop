...
export type PluginIpcResult<T> =
  ...
  | {
      readonly ok: false;
      readonly error: { readonly code: PluginIpcErrorCode; readonly message: string };
    };
...
export interface PluginIpcHandlers {
  ...
  reportRuntimeFailure(
    pluginId: string,
    message: string,
  ): Promise<PluginIpcResult<InstalledPluginRecord>>;
  ...
  runStrategy(
    ...
    key: string,
    input: StrategyInput,
  ): Promise<PluginIpcResult<StrategyOutput>>;
}
...
export function createPluginIpcHandlers(
  ...
  return {
    ...
    reportRuntimeFailure: (pluginId, message) =>
      invoke(() => manager.recordRuntimeFailure(pluginId, message)),
    uninstall: (pluginId) =>
      invoke(async () => {
    ...
    runStrategy: (pluginId, key, input) =>
      invoke(() => runtime.runStrategy(pluginId, key, input), "PLUGIN_RUNTIME_UNAVAILABLE"),
  };
}

async function invoke<T>(
  operation: () => Promise<T>,
  code: PluginIpcErrorCode = "PLUGIN_OPERATION_FAILED",
): Promise<PluginIpcResult<T>> {
  try {
    ...
    return {
      ...
      error: {
        code,
        message:
          error instanceof Error && error.message.trim() ? error.message : "Plugin runtime failed.",
      },
    };
  }
}
