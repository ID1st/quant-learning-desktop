  ...
  async () => {
    ...
    try {
      ...
      await pool.query(
        ...
        [...messageIds, { code: "123456", expiresInMinutes: 10 }],
      );
      ...
      assert.equal(
        rows.rows.every((row) => row.status === "SENT"),
        true,
      );
      assert.equal(
        rows.rows.every((row) => row.claim_token === null),
        true,
      );
      assert.equal(
      ...
      assert.equal(rows.rows.find((row) => row.id === messageIds[2])?.attempt_count, 2);
    } finally {
      await pool.end();
