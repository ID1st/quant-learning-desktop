...
import { createMarketDataGateway, createMarketDataProviderRegistry } from "../apps/desktop/src/features/marketData/marketDataProviderGateway.ts";
...
const outputPath = path.join(path.resolve(__dirname, ".."), "docs", "generated", "market-data-stability-drill-latest.json");
const symbols = [
...
function createStockSdkGateway() {
  const stockSdk = createStockSdkGatewayProvider(
    createStockSdkGatewayProviderOperations(undefined, { tencentBars: createTencentFinanceBarsOperations() }),
...
await runCheck("live.quote.cn-hk-us", async () => {
  ...
  if (!result.ok || result.data.length !== symbols.length) throw new Error(result.ok ? "Quote batch is incomplete." : result.error.message);
  return { provider: result.provider, upstream: result.health.upstream ?? null, rows: result.data.length, triedProviders: result.triedProviders };
});
...
for (const symbol of symbols) {
  ...
  await runCheck(`live.intraday.${symbol.market}.1m`, async () => {
    const { gateway } = createStockSdkGateway();
...
await runCheck("drill.forced-primary-failure-falls-back-to-stock-sdk", async () => {
  ...
  const result = await drillGateway.fetchHistoricalBars({ market: "CN", symbol: "600519.SH", timeframe: "1d", count: 120 });
  if (!result.ok || result.provider !== "stock-sdk" || result.triedProviders.join(",") !== "yahoo-finance,stock-sdk") {
    throw new Error(result.ok ? "Fallback provider order was not preserved." : result.error.message);
  }
  return { simulatedFailure: "yahoo-finance", provider: result.provider, upstream: result.health.upstream ?? null, rows: result.data.length, triedProviders: result.triedProviders };
});

await runCheck("drill.failed-refresh-retains-real-history-snapshot", async () => {
  const { gateway } = createStockSdkGateway();
  const initial = await gateway.fetchHistoricalBars({ market: "HK", symbol: "00700.HK", timeframe: "1d", count: 120 });
  if (!initial.ok) throw new Error(initial.error.message);
  const snapshot = JSON.stringify(initial.data);
  const failedGateway = createMarketDataGateway(createMarketDataProviderRegistry([createDeliberatelyFailingHistoryProvider()]), ["yahoo-finance"]);
  const refresh = await failedGateway.fetchHistoricalBars({ market: "HK", symbol: "00700.HK", timeframe: "1d", count: 120 });
  if (refresh.ok || JSON.stringify(initial.data) !== snapshot) throw new Error("A failed refresh changed the retained history snapshot.");
  return { retainedRows: initial.data.length, retainedProvider: initial.provider, refreshError: refresh.error.code };
});
...
async function loadBars(gateway, kind, request) {
  const result = kind === "historical" ? await gateway.fetchHistoricalBars(request) : await gateway.fetchIntradayBars(request);
...
async function runCheck(name, operation, onError) {
  ...
  try {
    ...
    checks.push({ name, status: "passed", latencyMs: Math.round(performance.now() - startedAt), data });
  } catch (error) {
    const observation = onError?.(error);
    if (observation) {
      checks.push({ name, status: "observed", latencyMs: Math.round(performance.now() - startedAt), observation });
      return;
    }
    checks.push({ name, status: "failed", latencyMs: Math.round(performance.now() - startedAt), error: error instanceof Error ? error.message : String(error) });
  }
}
...
function createDeliberatelyFailingHistoryProvider() {
  let failed = false;
  const capability = { realtimeQuote: false, historicalBars: true, intradayBars: false, websocket: false, batchQuote: false, markets: ["US", "HK", "CN"], timeframes: ["1d", "1w"], delayLevel: "unknown" };
  return {
    id: "yahoo-finance",
    ...
    async getHealth() {
      return {
        ...
        message: failed ? "Deliberate fallback drill failure." : "Ready for deliberate fallback drill.",
