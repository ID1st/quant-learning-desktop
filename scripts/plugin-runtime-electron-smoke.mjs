...
if (!existsSync(electron) || !existsSync(entry)) {
  throw new Error(
    "Electron runtime smoke prerequisites are missing. Run the desktop Electron build first.",
  );
}
...
if (exitCode !== 0 || !stdout.includes('"ok":true')) {
  throw new Error(
    `Electron plugin runtime smoke test failed. stdout=${stdout.trim()} stderr=${stderr.trim()}`,
  );
}
